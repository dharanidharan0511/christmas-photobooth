...
function applySchemeAndAccent(scheme: ColorScheme, accent: AccentVariant) {
  ...
  // Tailwind darkMode uses the `dark` class OR [data-theme="dark"].
  // Paper is still a light scheme for Tailwind — add "light" class for it.
...
function getInitialScheme(): ColorScheme {
  ...
  if (stored !== "light") return stored; // explicit preference stored
  // fall back to system preference for first visit
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Apply to DOM whenever either changes
  useEffect(() => {
