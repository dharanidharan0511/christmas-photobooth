...
/**
 * Global single-key shortcuts — must be mounted once inside a Router.
...
export function useKeyboardShortcuts() {
  ...
  useEffect(() => {
    function handler(e: KeyboardEvent) {
      // ⌘K / Ctrl+K always opens help (even while typing)
      ...
      switch (e.key) {
        case "h":
          navigate("/dashboard");   // Workbench — everyone lands here now
          break;
        case "c":
          navigate("/code-repos");
          break;
        case "d":
          navigate("/docflo/agents");
          break;
        case "a":
  ...
  }, [navigate, cycleScheme, openShortcuts, closeShortcuts, isOpen, isAdmin]);
}
