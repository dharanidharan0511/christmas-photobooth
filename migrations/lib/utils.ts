/** Tiny className joiner — no cva/clsx dependency. Used wherever the design
 * system's ported components expect a `cn()` helper. */
...
export function rolesFromCatalog<T extends { id: string; name: string; projectId?: string | null }>(roles: T[]): T[] {
  ...
  return [...byName.values()]
    ...
    .filter((role): role is T => role !== undefined)
